...
graficaMunicipios <- function(mxCasesDF , numEstado,numMuni, muniTxt,aretirar, daysToAverage, saveToFile,pathToSave){
	edoCasesDF <- mxCasesDF[mxCasesDF$ENTIDAD_RES %in% numEstado &mxCasesDF$MUNICIPIO_RES %in% numMuni & mxCasesDF$RESULTADO == resultadoConfirmado,c("FECHA_INGRESO","RESULTADO")]
...
#	 plot
	...
	# 1. Open jpeg file
		setwd(pathToSave)
		png(paste(muniTxt, ".png", sep=""), width = 800, height = 600)
	# 2. Create the plot
	}
	plot(x = casos$RESULTADO_ACUM7D, y = casos$RESULTADO7D ,xlab = "Acumulados", ylab="Nuevos",main=muniTxt, log="xy")
	...
	if (saveToFile == TRUE) {
		# 3. Close the file
		setwd(pathToSave)
		write.csv(casos, paste(muniTxt,".csv", sep=""))
		dev.off()
	}
